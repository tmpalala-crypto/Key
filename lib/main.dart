...
class _HomePageState extends State<HomePage>{
  ...
  List<String> alerts=[];
  void autoAlert(String code,bool isFake){
    String t=DateTime.now().toString().substring(11,19);
    String m="${isFake?'🚨 FAKE':'✅ ORIGINAL'}: $code at $gps $t";
    setState(()=>alerts.insert(0,m));
    if(isFake){showDialog(context:context, barrierDismissible:false, builder:(c)=>AlertDialog(
      backgroundColor:Colors.red.shade700,
      shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(20)),
      title:Row(children:[Icon(Icons.warning_amber_rounded,color:Colors.white,size:35), SizedBox(width:8), Expanded(child:Text("AUTOMATIC ALERT!",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)))]),
      content:Column(mainAxisSize:MainAxisSize.min, children:[
        Image.asset('assets/logo.png',height:120, errorBuilder:(c,e,s)=>Icon(Icons.shield,size:80,color:Colors.white)),
        SizedBox(height:10),
        Text("🚨 COUNTERFEIT DETECTED!",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)),
        SizedBox(height:6),
        Text("$code\n$gps\n$t",style:TextStyle(color:Colors.white70,fontSize:11)),
        SizedBox(height:8),
        Container(color:Colors.white, padding:EdgeInsets.all(6), child:Text("⚠️ Auto-Alert sent to ALL + HQ",style:TextStyle(color:Colors.red,fontWeight:FontWeight.bold,fontSize:10))),
      ]),
      actions:[TextButton(onPressed:()=>Navigator.pop(c), child:Text("ALERT ALL ✅",style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold)))],
    ...
    return Scaffold(
      ...
      body:Padding(padding:EdgeInsets.all(16), child:Column(children:[
        ...
        Expanded(child:GridView.count(crossAxisCount:2,crossAxisSpacing:12,mainAxisSpacing:12, children:[
          ...
          _btn(Icons.report_problem,"REPORT FAKE",Colors.red.shade700,()=>autoAlert("FAKE-${DateTime.now().millisecond}",true)),
          _btn(Icons.notifications_active,"ALERTS (${alerts.length})",Colors.orange,(){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text("${alerts.length} auto-alerts logged")));}),
        ])),
        Container(padding:EdgeInsets.all(6), decoration:BoxDecoration(color:Colors.black, borderRadius:BorderRadius.circular(6)), child:Row(mainAxisAlignment:MainAxisAlignment.center, children:[Icon(Icons.security,color:Colors.greenAccent,size:12), SizedBox(width:5), Text("YOUR LOGO + AUTO-ALERT ACTIVE ON ALL",style:TextStyle(color:Colors.greenAccent,fontSize:8,fontWeight:FontWeight.bold))])),
      ])),
    );
  }
